...
- Known limitation: "generates it itself" is read from reflected metadata, which does not describe triggers. A `uuid`/`CHAR(36)` primary key populated by a `BEFORE INSERT` trigger reflects as a key with no default, so a row that omits it is rejected by the rule above and the trigger never runs — and because the `answers` lane only logs a rejected batch, the rows are dropped with a single log line. Give such a column a real `DEFAULT` (`gen_random_uuid()`) or have the upstream node supply the key.
