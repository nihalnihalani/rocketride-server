...
# Export to a specific backend with auth headers
rocketride otel --endpoint https://cloud.langfuse.com/api/public/otel \
  --headers "Authorization=Basic ${AUTH_VALUE}" --no-metrics
...
| `--headers <k=v,k=v>` | Headers sent with every OTLP request (collector auth). Defaults to `OTEL_EXPORTER_OTLP_HEADERS`. |
