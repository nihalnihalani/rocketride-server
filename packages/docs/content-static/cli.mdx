...
> Until then, pass `install-from: ./packages/client-python` (or any pip source
> that provides the subcommand) instead of installing from PyPI.
...
# Alternative: inline equivalent of the composite action, for a single file.
      ...
      # with "invalid choice: 'diff'". Until that release, install the client
      # package from a checkout (see `install-from` above); afterwards pin it:
      # pip install "rocketride==<release>".
