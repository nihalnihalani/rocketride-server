...
| `--include-layout` | Enumerate the layout churn (each node's `ui` block and the top-level `viewport`) that is hidden by default, as `ui.*` field changes. |
...
- **Layout** — each node's `ui` block and the `viewport` — is ignored by default
  ...
  a semantic change, so `diff` exits `0` on it. Pass `--include-layout` to see the
  individual `ui.*` coordinates.
...
`--json` emits one stable, sorted document — `nodes`, `edges`, and a `summary`
block with counts and the overall `has_semantic_changes` flag — ideal for feeding
another tool:
...
for the `files`, `cli-version`, `comment`, and `include-layout` inputs.
