...
> the same commands and flag names — the one exception is `eval`, which is
> currently Python-only. Output is plain, line-oriented text;
...
two paths that exit `2` before any case runs — a spec that fails to parse or
validate, and a server the CLI cannot reach — write the shared
